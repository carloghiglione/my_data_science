# -*- coding: utf-8 -*-
"""
Created on Fri Jul  1 15:29:40 2022
...
crit = 'gini'  # 'entropy'
max_depth = 3
mod_rf = RandomForestClassifier(n_estimators=n_estim, criterion=crit, max_depth=max_depth, oob_score=True)
...
mod_rf_cv = RandomForestClassifier(n_estimators=n_estim, criterion=crit, max_depth=max_depth, oob_score=True)

acc_list = cross_val_score(mod_rf_cv, X_train, y_train, cv=cv, scoring='accuracy')
acc_cv = np.mean(acc_list)
acc_std_cv = np.std(acc_list)

print('Accuracy Random Forest')
print(f'Train: {acc_train}')
print(f'OOB Score: {mod_rf.oob_score_}')  
