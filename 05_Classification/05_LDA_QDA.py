...
ax.set_title('Confusion Matrix Norm, LDA (Test)')
plt.tight_layout()
...
ax.set_title('Fitted Class Space LDA')
fig.tight_layout()
...
###################################################################################
# QDA
# Requires each group to be Multivariate Gaussian, all groups may have different Covariance Matrix

priors = np.ones(3)/3
mod_qda = QuadraticDiscriminantAnalysis(priors=priors) # can introduce shrinkage with shrinkage='auto' 
mod_qda_noprior = QuadraticDiscriminantAnalysis()      # priors are empirical frequencies
...
y_hat_train = mod_qda.predict(X_train)
y_hat_test = mod_qda.predict(X_test)
acc_train = accuracy_score(y_train, y_hat_train)
acc_test = accuracy_score(y_test, y_hat_test)
...
# Cross-Validation
cv = KFold(n_splits=10, shuffle=True, random_state=1)
mod_qda_cv = QuadraticDiscriminantAnalysis(priors=priors)

acc_list = cross_val_score(mod_qda_cv, X_train, y_train, cv=cv, scoring='accuracy')
acc_cv = np.mean(acc_list)
acc_std_cv = np.std(acc_list)
...
fig, ax = plt.subplots(1,1, figsize=(5,4))
sns.heatmap(cm_norm, square=True, cmap='Blues', annot=True, ax=ax)
ax.set_title('Confusion Matrix Norm, QDA (Test)')
plt.tight_layout()
...
z = mod_qda.predict(np.c_[xx0_g.ravel(), xx1_g.ravel()])
z = z.reshape(xx0_g.shape)

fig, ax = plt.subplots(1,1)
ax.pcolormesh(xx0, xx1, z, cmap='tab10')
sns.scatterplot(X[:,0], X[:,1], hue=y, palette='tab10', ax=ax)
ax.set_title('Fitted Class Space QDA')
fig.tight_layout()
